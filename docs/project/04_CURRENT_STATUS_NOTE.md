...
- 한국어 `/currency-converter`는 달러·엔화·유로·원화 환산 의도, 22개 지원 통화, 참고 환율과 수수료 차이, ZWL 미지원 범위를 명확히 했다.
...
1. 2~3일 후 `/guides`, `/guides/jpg-images-to-one-pdf`, `/image-to-pdf`, `/pdf-merge`, `/pdf-split`, `/pdf-organize`, `/pdf-to-png`, `/pdf-tools`, 한국어·영어 `/word-counter` 상태를 다시 확인한다.
2. `www` sitemap의 발견 URL 수가 105에서 107로 갱신되는지 확인한다.
3. 2~4주 뒤 `이미지 도트 변환`, `korean word counter`, `word counter korean`, `jpg pdf 변환`의 노출·클릭 변화를 비교한다.
4. 위 상태가 안정되기 전에는 AdSense 재검토를 서두르지 않고, 가이드 색인과 실제 검색 유입을 먼저 확인한다.
