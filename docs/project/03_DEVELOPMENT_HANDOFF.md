...
- 사용자 확인 사항: 2026-09-16 서류 제출·스캔 PDF·PDF 오류 해결·JPG PDF 변환 가이드의 색인 요청 완료, `www` sitemap 재제출 성공. `/pixel-art`와 `/currency-converter`는 이미 색인되어 있었고 배포 후 실시간 검사에서 접근·색인 가능과 Breadcrumbs 정상 상태를 확인한 뒤 각각 재색인을 요청했다. 영어 `/word-counter?lang=en`과 한국어 기본 `/word-counter`는 발견됐지만 아직 색인되지 않은 상태였고, 두 URL 모두 실시간 접근·색인 가능을 확인한 뒤 우선 크롤링 대기열에 등록했다. `/image-to-pdf`도 sitemap과 이미지 형식 가이드에서 발견됐지만 아직 크롤링되지 않아, 실시간 검사 통과 후 색인을 요청했다. 기존 `/pdf-converter`는 자기 URL이 Google 선택 canonical인 색인 완료 페이지이고 실시간 검사도 통과했으므로 리디렉션하지 않고 유지한다. 새 `/pdf-merge`는 발견 후 아직 크롤링되지 않았으며 실시간 검사 통과 후 우선 크롤링 대기열에 등록했다. `/pdf-split`도 sitemap과 페이지 추출 검증 가이드에서 발견됐지만 아직 크롤링되지 않아, 실시간 검사 통과 후 색인을 요청했다. `/pdf-organize`도 같은 가이드와 sitemap에서 발견 후 크롤링 대기 상태였으며 실시간 검사 통과 후 우선 대기열에 등록했다. `/pdf-to-png`도 sitemap 발견 후 크롤링 대기 상태였고 참조 페이지 보고만 불러오지 못했으나, 실시간 접근·색인 가능을 확인해 색인을 요청했다. Search Console의 선택된 3개월 보고서는 2클릭·29노출이며 `jpg pdf 변환`과 `/pdf-converter`에서 각각 1클릭을 확인했다. 비-www와 HTTP는 운영에서 www·HTTPS로 이미 308 이동한다. `?lang=ko` 영구 이동 시도는 Vercel 설정 검증에서 거절되어 운영에 반영되지 않았고, 기존의 쿼리 없는 한국어 canonical 지정을 유지한다. 사이트맵 상태는 성공이며 발견 URL 수는 `www` 105개·비-www 90개다. 독립 스티커북: https://github.com/ionic2001/diorama-stickerbook.
