...
- 다음 작업: Search Console 우선 크롤링을 요청한 기존 검증 가이드 3개와 서류 제출·스캔 PDF 가이드 2개의 색인·검색 의도·관련 도구 이동을 확인한다. 재제출 뒤 현재 106개로 확장된 sitemap의 발견 URL 수가 이전 60·90에서 갱신되는지도 확인한다. 페이지별 개별 추출·ZIP, 드래그 정리, 내용 추가·양식·OCR는 후속 기능이다.
- 사용자 확인 사항: 2026-09-16 서류 제출·스캔 PDF 가이드의 색인 요청 완료, `www` sitemap 재제출 성공. 독립 스티커북: https://github.com/ionic2001/diorama-stickerbook.
