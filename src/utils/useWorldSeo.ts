...
export function useWorldSeo(language: Language) {
  useEffect(() => {
    ...
    const tr = worldText[language];
    const title = `${worldGuide[language].title} | QK Tool Hub`;
    const description = worldGuide[language].description;
    const base = 'https://qktoolhub.com/world-clock';
    ...
    const schema = set('#world-schema', 'script', { id: 'world-schema', type: 'application/ld+json' });
    schema.textContent = JSON.stringify({ '@context': 'https://schema.org', '@type': 'WebApplication', name: title, description, url, inLanguage: language, applicationCategory: 'UtilitiesApplication', operatingSystem: 'Any',  offers: { '@type': 'Offer', price: '0', priceCurrency: 'USD' }, featureList: [tr.clock, tr.alarm, tr.stopwatch, tr.timer, tr.focus] });
