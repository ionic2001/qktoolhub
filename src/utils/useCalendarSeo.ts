...
export function useCalendarSeo(language: Language) {
  useEffect(() => {
    ...
    const set = (selector: string, tag: string, attributes: Record<string, string>) => {
      ...
      undo.push(() => {
        ...
        else target.replaceWith(original);
      });
      return node;
    };

    const tr = calendarText[language] || calendarText.ko;
    const guide = calendarGuide[language] || calendarGuide.ko;
    const title = `${guide.title} | QK Tool Hub`;
    const description = guide.description;
    const base = 'https://qktoolhub.com/calendar';
    ...
    for (const [key, value] of Object.entries({
      ...
      set(`meta[property="${key}"]`, 'meta', { property: key, content: value });
    }

    set('link[rel="canonical"]', 'link', { rel: 'canonical', href: url });

    for (const lang of ['ko', 'en', 'ja', 'zh', 'es', 'x-default']) {
      set(`link[hreflang="${lang}"]`, 'link', {
        rel: 'alternate',
        hreflang: lang,
        href: lang === 'x-default' ? base : `${base}?lang=${lang}`,
      });
    }
    ...
    const schema = set('#calendar-schema', 'script', { id: 'calendar-schema', type: 'application/ld+json' });
