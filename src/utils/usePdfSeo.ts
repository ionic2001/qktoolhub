...
export function usePdfSeo(language: Language) {
  useEffect(() => {
    ...
    const base = 'https://qktoolhub.com/pdf-converter';
    ...
    const schema = set('#pdf-schema', 'script', { id: 'pdf-schema', type: 'application/ld+json' });
    schema.textContent = JSON.stringify({ '@context': 'https://schema.org', '@graph': [
      { '@type': 'WebApplication', name: title, description, url, inLanguage: language, applicationCategory: 'UtilitiesApplication', operatingSystem: 'Any', offers: { '@type': 'Offer', price: '0', priceCurrency: 'USD' } },
      { '@type': 'FAQPage', inLanguage: language, mainEntity: guide.faqs.map(item => ({ '@type': 'Question', name: item.q, acceptedAnswer: { '@type': 'Answer', text: item.a } })) }
    ] });
