...
export function GuideContent({ path }: { path: string }) {
  ...
  return <main className="guide-article">
    ...
    <article>
      ...
      {guide.downloads && <section className="guide-section"><h2>검증 표본 다운로드</h2><p>아래 파일은 이 글의 출력 확인에 사용한 작은 표본입니다. 개인 정보가 없는 테스트 문서입니다.</p><div className="guide-downloads">{guide.downloads.map(item => <a key={item.href} href={item.href} download><strong>{item.label}</strong><span>{item.detail}</span></a>)}</div></section>}
