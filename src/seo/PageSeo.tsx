...
import { allPagePaths, canonicalUrl, languages, origin, pageMeta, pageSchema } from './catalog';
...
export function PageSeo() {
  ...
  useEffect(() => {
    ...
    document.documentElement.lang = isGuide ? 'ko' : language;
    const known = allPagePaths.includes(pathname);
    ...
    const meta = pageMeta(pathname, isGuide ? 'ko' : language);
    ...
    for (const [property, content] of Object.entries({ 'og:type': isGuide && pathname !== '/guides' ? 'article' : 'website', 'og:site_name': 'QK Tool Hub', 'og:title': meta.title, 'og:description': meta.description, 'og:url': meta.url, 'og:image': origin + '/og-image.png', 'og:image:alt': 'QK Tool Hub', 'og:locale': isGuide ? 'ko_KR' : ({ ko: 'ko_KR', en: 'en_US', ja: 'ja_JP', zh: 'zh_CN', es: 'es_ES' })[language] })) set(`meta[property="${property}"]`, 'meta', { property, content });
    ...
    set('#page-schema', 'script', { id: 'page-schema', type: 'application/ld+json' }).textContent = JSON.stringify(pageSchema(pathname, isGuide ? 'ko' : language));
  }, [pathname, language]);
  return null;
}
