...
import { guideByPath, guideLinksForTool } from '../guides/guideContent';
export function SeoSections({ path, language }: { path: string; language: Language }) {
  ...
  return <div className="seo-sections">
    ...
    {language === 'ko' && guideLinksForTool[path] && <nav aria-label="직접 검증한 활용 가이드"><h2>직접 검증한 활용 가이드</h2><ul>{guideLinksForTool[path].map(guidePath => { const guide=guideByPath(guidePath)!; return <li key={guidePath}><a href={guidePath}>{guide.title}</a><p>{guide.description}</p></li>; })}</ul></nav>}
